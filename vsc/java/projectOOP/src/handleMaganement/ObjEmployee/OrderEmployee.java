...
public class OrderEmployee extends Employee {
    ...
    public void createBill(Customer customer, ArrayList<OrderItem> products, ArrayList<Integer> quantities) {
    ...
    public void changeOrder(Bill bill, OrderItem newProduct, int newQuantity) {
        if ("Đang Đặt".equalsIgnoreCase(this.Status)) {
        } else {
    ...
    private void modifyProduct(Bill bill) {
        ...
        if (!found) {
            ...
            OrderItem newProduct = listOrder.findProductByCode(newProductCode);
