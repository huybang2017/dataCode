...
class MembershipManager {
    public static double calculateDiscount(Customer customer, double totalAmount) {
        if (customer.isCustomerType() && isBirthday(customer)) {
    ...
    // public static Customer registerMember(String name, String telephoneNumber,
    // int age, String address, String gender) {
    // // Thực hiện đăng ký thành viên, có thể thêm các bước khác nếu cần
    // Calendar today = Calendar.getInstance();
    // int currentMonth = today.get(Calendar.MONTH) + 1;
    // return new Customer(name, telephoneNumber, age, address, gender, true,
    // currentMonth, 0);
    // }
